...
* **Pool com fallback obrigatório, resolvido POR REQUEST.** A origem boa é a coleção `[annotate] pool_collection` do corpus; como o s11 **apaga as coleções** a cada carga, hoje o que vale é o fallback (`[annotate] pool_fallback_*`). A origem ativa aparece no `/api/health` e no painel do admin — pool trocando em silêncio é mentira de interface. Coleção existente mas **vazia** conta como ausente.
* **O fallback amostra com passo constante, não com `LIMIT`.** O universo é gravado agrupado por fonte: `ORDER BY id LIMIT 500` devolve 500 linhas todas de `arena140k`. O passo sobre o conjunto inteiro devolve a proporção real (medido: 393 wildchat_pt, 83 aya, 19 arena140k, 4 oasst, 1 wildchat_en). Custo: ~95 ms a lista completa (`com_uids=True`) contra **0,2 ms** só a contagem (`LIMIT` dentro da subconsulta, índice de cobertura, sem tocar a tabela) — é por isso que o `/api/health` nunca pede a lista.
