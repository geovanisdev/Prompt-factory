...
> **English (primary): [README.md](README.md).** Esta é a versão em português, mantida em paralelo — os números são os mesmos e um teste da suíte cobra que continuem sendo.
...
& "$env:USERPROFILE\.local\bin\uv.exe" run pytest -q # 1.471 testes, ~2 min (medido em 2026-09-05)
...
tests/              1.471 testes, ~110 s (5 marcados `slow` ficam fora do CI)
